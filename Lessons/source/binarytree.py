...
class BinaryTreeNode(object):
    ...
    def height(self):
        ...
        # check for leaf case for time I almost forgot
        if self.is_leaf():
            return 0
...
class BinarySearchTree(object):
    ...
    def delete(self, item):
        ...
        # not found case
        if node == None:
            raise ValueError("Node wasn't found when trying to delete a node")
        # node is root case
        if node.height() == 0:
            self.root = None
            return
        # only find parent once for the last 2 cases
        parent = self._find_parent_node_iterative(item)
        # node is leaf case
        if node.is_leaf():
            # find which side the node is and remove it
