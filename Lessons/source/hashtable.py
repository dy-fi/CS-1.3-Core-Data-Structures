...
class HashTable(object):
    ...
    def length(self):
        ...
        Best and worst case running time: O(n^2) because we are using the linked list
        .length() method"""
    ...
    def contains(self, key):
        ...
        Worst case running time: still O(1) if the load factor is not over 1"""
    ...
    def set(self, key, value):
        ...
        Best case running time: O(1) 
        Worst case running time: O(k) k = bucket keys"""
        # Find the bucket the given key belongs in
        index = self._bucket_index(key)
        bucket = self.buckets[index]
        # Find the entry with the given key in that bucket, if one exists
    ...
    def delete(self, key):
        ...
        Best case running time: O(1) if key is 1 (head)
        Worst case running time: O(n) if key is len(self) (tail)"""
        # Find the bucket the given key belongs in
        index = self._bucket_index(key)
        bucket = self.buckets[index]
        # Find the entry with the given key in that bucket, if one exists
    ...
    def _resize(self, new_size=None):
        ...
        Best and worst case running time: O(n)
        Best and worst case space usage: O(n^2)"""
